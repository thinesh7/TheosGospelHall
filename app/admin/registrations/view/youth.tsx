...
export default function RegistrationsViewYouthRoute() {
  const ref = useAdminBackHandler();
  const meta = ADMIN_ROUTE_META['/admin/registrations/view/youth'];

  return (
    <>
      <Stack.Screen options={{ headerTitle: () => <AdminHeaderTitle title={meta.title} subtitle={meta.subtitle} /> }} />
      <RegistrationsAdmin ref={ref} programId="youth" />
    </>
  );
}
