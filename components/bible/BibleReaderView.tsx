...
export interface BibleReaderViewProps {
  ...
  isBilingual: string;
  secondaryVersion: string;
  // Replaces every router.back() call the old routed screen made — the
  // route wrapper (app/bible-reader.tsx, native's path) passes router.back()
  // itself; the Bible tab's web path passes its own raw-history goBack()
  // instead so Reader never needs its own expo-router push/pop cycle. See
  // the comment on BibleNav.view in app/(tabs)/bible.tsx for why that split
  // matters.
  onClose: () => void;
}

export default function BibleReaderView({
  bookId, chapter, version: initialVersion, isBilingual: isBilingualParam, secondaryVersion: secondaryVersionParam, onClose,
  ...
  const [selectedBook, setSelectedBook] = useState<any>(BOOKS.find(b => b.id === Number(bookId)));
  const [selectedChapter, setSelectedChapter] = useState(Number(chapter));
  const [version, setVersion] = useState(initialVersion || 'TAMOVR');
  const versionRef = useRef(initialVersion || 'TAMOVR');
  const [isBilingual, setIsBilingual] = useState(isBilingualParam === '1');
  const bilingualEligible = isBilingualParam === '1';
  ...
  return (
    <View style={[styles.container, { backgroundColor: c.bg }]} {...panResponder.panHandlers}>
      <View style={[styles.header, { backgroundColor: c.headerBg, paddingRight: 16 + insets.right, paddingTop: insets.top + 12 }]}>
        <TouchableOpacity onPress={onClose} style={styles.backBtn}>
          {/* Web: an explicit close (X) reads unambiguously as "exit the
              reader, back to Chapter Selection" — same onClose action
      ...
      <View style={[styles.bottomNav, { backgroundColor: c.surface, borderTopColor: c.divider, paddingBottom: insets.bottom + 16 }]}>
        ...
        <TouchableOpacity style={styles.navChapterInfo} onPress={onClose}>
